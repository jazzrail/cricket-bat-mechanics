document.getElementById("btn-calc").addEventListener("click", (e) => {
	...
	const handleLen = parseFloat(
		document.querySelector("#handleLen").value.toFixed(3)
	);
	...
	const bladeLen = parseFloat(
		document.querySelector("#bladeLen").value.toFixed(3)
	);
	if (document.getElementById("bladeLen").value.length == 0) {
		alert("Please enter integer or decimal in each of the fields");
		document.classList.remove("empty-field");
	}
	const batMass = parseFloat(
		document.querySelector("#batMass").value.toFixed(3)
	);
	...
	const balPoint = parseFloat(
		document.querySelector("#balPoint").value.toFixed(3)
	);
